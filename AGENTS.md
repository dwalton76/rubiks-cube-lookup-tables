...
| `777-daisy-perfect` | Three 70^5 daisy tables (optional) | daisy with `use_perfect_tables` |
| `777-solve-perfect` | Native-orientation twins of daisy-perfect | `--native-only` for 9x9+ |
| `777-phase5` | Older graph/perfect-hash UD obliques | leftover graph path |

`777-daisy-perfect` / `777-solve-perfect` are 70^5 bytes **each**. Comments in the Makefile call them out as huge.
...
Sidecar `*.cost-only.bin.json` (`dense-multiset-cost-v1`):
