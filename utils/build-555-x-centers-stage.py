...
from rubikscubennnsolver.RubiksCube555 import RubiksCube555, solved_555, x_centers_555
...
x_centers_minus_dead_centers = []
for x in x_centers_555:
    if x not in dead_centers:
        x_centers_minus_dead_centers.append(x)

filename = "/storage/dwalton76/lookup-table-5x5x5-step16-centers-stage-x-center-only.txt"
...
with open(filename, "r") as fh:
    for x in range(lines_to_process):
        ...
        for pos, value in zip(x_centers_minus_dead_centers, state):
            cube.state[pos] = value
        # cube.print_cube()

        lr_state = cube.lt_LR_x_centers_stage.state()
        lr_cost = cube.lt_LR_x_centers_stage.steps_cost(lr_state)
        ud_state = cube.lt_UD_x_centers_stage.state()
        ud_cost = cube.lt_UD_x_centers_stage.steps_cost(ud_state)
        # log.info(f"INIT LR t-centers cost {lr_cost}, UD t-centers cost {ud_cost}, cost {cost}")
