...
edges_filename = "lookup-table-5x5x5-step900-edges.txt"
    ...
    with open(edges_filename, "r") as fh:
        for (line_number, line) in enumerate(fh):
            ...
            for centers_solution in centers_solutions[centers_state]:
                ...
                if edges_state != "OOopPPQQqrRRsSSTTtuUUVVvWWwxXXYYyzZZ":
                    to_write.append("{}:{}".format(edges_state, " ".join(reverse_steps(steps_to_scramble + centers_solution))))
                    to_write_count += 1
                    #fh_new.write("{}:{}\n".format(edges_state, " ".join(reverse_steps(steps_to_scramble + centers_solution))))
                #log.info(state)
