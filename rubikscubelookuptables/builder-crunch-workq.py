...
from rubikscubelookuptables.buildercore import SORT_BUFFER_SIZE, TMPDIR, WRITE_BATCH_SIZE, supported_sizes
            ...
            if to_write_count >= WRITE_BATCH_SIZE:
                ...
                if lines_since_last_merge >= 100000000:
                    ...
                    subprocess.check_output(
                        "LC_ALL=C nice sort --merge --buffer-size=%s --temporary-directory=%s --output %s.all %s.*"
                        % (SORT_BUFFER_SIZE, TMPDIR, outputfilebase, outputfilebase),
                        shell=True,
                    )
